...
public abstract class MathBuilder extends Builder {
  ...
  public void addKern(Dimen kern) {
    addNode(new HKernNode(kern));
  }

  public void addSkip(Glue skip) {
    addNode(new HSkipNode(skip));
  }
  ...
  private FractionNoad makeFraction(NoadList num, Field den) {
    return (fractLeft != Delimiter.NULL)
        ? new DelFractionNoad(new NoadListField(num), den, thickness, fractLeft, fractRight)
  ...
  protected void specialShow(Log log, int depth, int breadth) {
    ...
    if (alreadyFractioned()) {
      ...
      CntxLog.addItem(
          log, makeFraction(fakeLeftDelim(numerator), EmptyField.FIELD), depth, breadth);
    }
  }
