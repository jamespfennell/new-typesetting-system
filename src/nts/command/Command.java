...
public abstract class Command extends CommandBase implements Serializable, Loggable {
  ...
  public void addExpandable(Log log) {
  ...
  public Command meaning(boolean expOK) {
    return this;
  }
  ...
  public Num getNumValue() {
    if (hasDimenValue()) return Num.valueOf(getDimenValue().toInt(DIMEN_DENOMINATOR));
    if (hasMuDimenValue()) {
  ...
  public Dimen getDimenValue() {
    return (hasGlueValue()) ? getGlueValue().getDimen() : Dimen.NULL;
  }

  public Dimen getMuDimenValue() {
    return (hasMuGlueValue()) ? getMuGlueValue().getDimen() : Dimen.NULL;
  }
