...
public class FontPrim extends TypoAssignPrim implements TokenList.Provider {
  ...
  protected void assign(Token src, boolean glob) {
    ...
    Num scale = (size != Dimen.NULL) ? Num.NULL : scanScaleSpec();
    getConfig().enableInput(inpEnbl);
    FontMetric metric = getTypoHandler().getMetric(name, size, scale, ident, tok);
    if (metric != FontMetric.NULL) tok.define(new SetFont(metric), glob);
  }
