...
public class WritePrim extends BuilderPrim {
  ...
  private static final Token FROZEN_END_WRITE =
      new FrozenToken("endwrite", new Macro(MacroBody.EMPTY, PrefixPrim.OUTER));

  private static final Token[] END_TOKS = {RightBraceToken.TOKEN, FROZEN_END_WRITE};
  ...
  protected void write(int num, TokenList list) {
    ...
    else if ((output = get(num)) == Log.NULL) output = normLog.startLine();
  ...
  protected abstract static class FileNode extends BaseNode {
    ...
    protected Log addName(Log log, String name) {
      ...
      return (num < 0)
          ? log.add('-')
          : (num > getConfig().getIntParam(Prim.INTP_MAX_FILE_CODE)) ? log.add('*') : log.add(num);
    }
