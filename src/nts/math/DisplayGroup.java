...
public class DisplayGroup extends FormulaGroup {
  ...
  public DisplayGroup() {
    super(new DisplayBuilder(currLineNumber()));
  }
  ...
  public void stop() {
    ...
    if (success) {
      ...
      if (eqNoBox != Node.NULL) {
        ...
        if (!eqNoWidth.isZero()) {
          eqNoAdd = style.getDimPar(ConvStyle.DP_MATH_QUAD).plus(eqNoWidth);
          sideBySide = true;
        }
      }
    }
    NodeEnum mig =
        (bld.wantsMigrations()) ? list.extractedMigrations().nodes() : NodeList.EMPTY_ENUM;
    ...
    Node kernBetween =
        (sideBySide)
            ? new ChrKernNode(width.minus(boxWidth).minus(eqNoWidth).minus(delta))
            : Node.NULL;
    int belowSkipParam;
    if (eqNoLeft) {
  ...
  public static void resumeAfterDisplay() {
    ...
    Builder par = new ParBuilder(currLineNumber(), TypoCommand.getTypoConfig().getLanguage());
    Builder.push(par);
    skipOptExpSpacer();
    bld.buildPage();
  }
  ...
  public static class DisplayPacker extends TypoCommand.HBoxPacker {
    ...
    public HBoxNode packHBox(NodeList list, Dimen additional, Dimen desired, BoolPar broken) {
      ...
      if (pack.getReport() == SizesEvaluator.OVERFULL && !additional.isZero()) {
