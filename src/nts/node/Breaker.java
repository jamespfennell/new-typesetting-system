...
public abstract class Breaker {
  ...
  protected class Fitness implements Loggable {
    ...
    public Break makeFirst(int lineNo) {
      return new Break(0, 0, lineNo, this, false, 0, maxSerial++, NULL_BREAK, new NetDimen());
    }
  ...
  protected void resetDemerits() {
    minDem = AWFUL_BAD;
    for (Fitness fit = FITNESS_HEAD; fit != NULL_FITNESS; fit = fit.next) fit.reset();
  }
  ...
  */
  ...
  public void breakToLines(NetDimen background, int threshold, boolean finPass) {
    ...
    if (passNodes()) tryBreak(nodeCount, Node.EJECT_PENALTY, true, Dimen.ZERO, true);
    ...
    if (n > 0) {
      lineBreaks = new Break[n];
      currLineIndex = 1;
      for (Break brk = best; brk != NULL_BREAK; brk = brk.prev) lineBreaks[--n] = brk;
    }
  }
  ...
  protected NodeList makeList(Break before, Break after) {
    ...
    int end = after.index;
    NodeList list;
    if (lastPostBreak != NodeEnum.NULL) list = new NodeList(lastPostBreak);
    ...
    for (int i = 0; stillNodeAt(i); i++) {
      ...
      if (!node.isKernBreak() || stillNodeAt(i + 1) && nodeAt(i + 1).canFollowKernBreak()) {
        ...
        if (pen < Node.INF_PENALTY) {
          ...
          tryBreak(
              i,
              (pen < Node.EJECT_PENALTY) ? Node.EJECT_PENALTY : pen,
              node.isHyphenBreak(),
              preWidth,
              false);
  ...
  protected void tryBreak(int idx, int pen, boolean hyphen, Dimen preWidth, boolean last) {
    ...
    while (iterator.hasNext()) {
      ...
      int badness;
      Fitness fitness;
      Dimen diff = shape.getWidth(brk.lineNo).minus(currWidth.getNatural());
      // D*/ System.out.print("> diff = " + diff);
      if (diff.moreThan(0)) {
        badness =
            (currWidth.getMaxStrOrder() > Glue.NORMAL)
                ? 0
    ...
    for (Fitness fit = FITNESS_HEAD; fit != NULL_FITNESS; fit = fit.next) {
      if (fit.fits(limit)) {
        if (delta != NetDimen.NULL) {
          ...
          Break best = fit.makeBest(idx, cnt, hyphen, new NetDimen());
          iterator.add(best);
          traceBreak(best);
        }
        // D*/ traceBreakList();
  ...
  protected abstract void traceBreak(Break brk);
}
