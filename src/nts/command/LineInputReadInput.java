...
public class LineInputReadInput implements ReadInput {
  ...
  /**
   * Creates an |LineInputReadInput| with given input and parametrization object.
   *
   * @param input the underlying reader.
   * @param inpHandler the parametrization object.
   */
  public LineInputReadInput(LineInput input, InputHandler inpHandler, int num) {
    this.input = input;
  ...
  public Tokenizer nextTokenizer(Token def, int ln) {
    ...
    return (line == LineInput.EOF) ? Tokenizer.NULL : inpHandler.makeTokenizer(line, desc, true);
  }
