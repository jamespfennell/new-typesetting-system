...
/**
 * The |TFtoPL| class simply puts other useful classes together and behave like traditional tftopl
 * program.
 *
 * <p>So far it does not any special locating of source tfm file --- it simply tries to open the
 * file given as argument. The output is directed to the standard output. It should be improved if
 * the strict compatibility with tftopl is required.
 */
...
public class TFtoPL {
  ...
  /** |Diag| implements |TeXFmDiagnostic| to provide error messaging during loading. */
  private static class Diag implements TeXFmDiagnostic {
    ...
    public void fatal(String s) {
      ...
      System.err.println("Sorry, but I can't go on;" + " are you sure this is a TFM?");
    }
    ...
    /**
     * Returns |false| to indicate that the loader should proceed further after normal (non fatal)
     * errors.
     *
     * @return |false|.
     */
  ...
  /**
   * The |main| program tries to open the file which name given as the first argument, to load the
   * content of file as a TeX Font Metric and list out its representation as a property list.
   *
   * @exception IOException if an I/O error ocurs or if the tfm file is malformed.
   */
  public static void main(String[] args) throws IOException {
    if (args.length > 0) {
      PrintWriterPLDumper dmp = new PrintWriterPLDumper(new PrintWriter(System.out, true));
