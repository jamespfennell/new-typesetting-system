  ...
  public final Action NORMAL =
      new Action() {
        ...
        public void exec(Builder bld, Token src) {
          if (bld.isEmpty() && !getTypoConfig().pendingOutput()) endMainLoop(dumping);
          else {
            backToken(src);
            bld.addBox(
                new HBoxNode(
                    new BoxSizes(
                        Dimen.ZERO, getConfig().getDimParam(DIMP_HSIZE), Dimen.ZERO, Dimen.ZERO),
                    GlueSetting.NATURAL,
                    NodeList.EMPTY));
            bld.addNode(
                new VSkipNode(
                    Glue.valueOf(Dimen.ZERO, Dimen.UNITY, Glue.FILL, Dimen.ZERO, Glue.NORMAL)));
