...
/**
 * Stack of |Tokenizer|s. It provides methods |push| for |Tokenizer|, array of |Token|s and single
 * |Token|. Method |nextToken| calls the same method of the |Tokenizer| on the top of stack.
 */
public class TokenizerStack {
  ...
  /**
   * Creates a |Tokenizer| for a portion of a |TokenList| and pushes it on the top of the stack.
   *
  ...
  /**
   * Creates a Tokenizer for a |TokenList| and pushes it on the top of the stack.
   *
   * @param list the |TokenList|.
   */
  ...
  /**
   * Creates a |Tokenizer| for a single |Token| and pushes it on the top of the stack.
   *
  ...
  /**
   * Gives the next |Token| from the |Tokenizer| stack. It calls the |nextToken| method of the
   * |Tokenizer| on the top of the stack. If the |Tokenizer| on top finishes (returns |Token.NULL|)
   * it is poped out and the next stack top |Tokenizer| is asked. When the stack is empty it returns
   * |Token.NULL| to indicate that the whole stack is finished.
   *
  ...
  public synchronized Token nextToken(BoolPar canExpand) {
    ...
    while (top != Tokenizer.NULL) {
      if ((tok = top.nextToken(canExpand)) != Token.NULL)
        return (checker != InpTokChecker.NULL) ? checker.checkToken(tok, canExpand) : tok;
      if (pop().close() && checker != InpTokChecker.NULL) checker.checkEndOfFile();
    }
    canExpand.set(false);
    return Token.NULL;
  }
  ...
  public synchronized void endTopmostInput() {
    for (Tokenizer curr = top; curr != Tokenizer.NULL; curr = curr.getNext())
  ...
  public synchronized void show(ContextDisplay disp) {
    ...
    Tokenizer curr = top;
    if (curr != Tokenizer.NULL)
      for (; ; curr = curr.getNext()) {
        boolean bottom = (curr.enoughContext() || curr.getNext() == Tokenizer.NULL);
