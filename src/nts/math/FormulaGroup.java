...
public class FormulaGroup extends SimpleGroup {
  ...
  public FormulaGroup() {
    this(new FormulaBuilder(currLineNumber()));
  }
  ...
  public void stop() {
    ...
    if (necessaryParamsDefined())
      bld.addNodes(
          Conversion.madeOf(
                  builder.getList().noads(), new FormulaStyle(Noad.TEXT_STYLE, bld.willBeBroken()))
              .nodes());
  ...
  public static boolean necessaryParamsDefined() {
    if (!necessaryParamsDefined(ConvStyle.SYMBOL_FAMILY, FontMetric.ALL_MATH_SYMBOL_DIMEN_PARAMS)) {
      error("InsufSymFonts");
      return false;
    }
    if (!necessaryParamsDefined(
        ConvStyle.EXTENSION_FAMILY, FontMetric.ALL_MATH_EXTENSION_DIMEN_PARAMS)) {
      error("InsufExtFonts");
      return false;
    }
    return true;
  ...
  public static boolean necessaryParamsDefined(byte fam, int[] idxs) {
    for (byte s = TEXT_SIZE; s < NUMBER_OF_FONT_SIZES; s++)
      if (!MathPrim.getMathConfig().familyFont(s, fam).definesDimenParams(idxs)) return false;
    return true;
  }
  ...
  protected static class FormulaStyle extends ConvStyle {
    ...
    private void metricError(byte fam, CharCode code) {
      error("UndefFamily", esc(MathPrim.getMathConfig().familyName(fontSize, fam)), num(fam), code);
    }
      ...
      for (; ; ) {
        if (code != CharCode.NULL)
          for (byte s = fontSize; s >= TEXT_SIZE; s--) {
            ...
            if (node != Node.NULL) {
              Dimen size = node.getHeight().plus(node.getDepth());
    ...
    public Dimen getXHeight(byte fam) {
      return fetchFontMetric(fam).getDimenParam(FontMetric.DIMEN_PARAM_X_HEIGHT);
    }

    /* TeXtp[741] */
    public Dimen skewAmount(byte fam, CharCode code) {
      FontMetric metric = fetchFontMetric(fam);
      Num skewCharNum = metric.getNumParam(FontMetric.NUM_PARAM_SKEW_CHAR);
      if (skewCharNum != Num.NULL) {
        CharCode skewCode = Token.makeCharCode(skewCharNum.intVal());
    ...
    public Dimen delimiterSize(Dimen height, Dimen depth) {
      ...
      Dimen size = max.over(500).times(getConfig().getIntParam(INTP_DELIMITER_FACTOR));
    ...
    public Node getSpacing(byte left, byte right) {
      ...
      return (space.isZero()) ? Node.NULL : new NamedHSkipNode(muToPt(space), spacer.getName());
    }

    public Num getPenalty(byte left, byte right) {
      return (penalties) ? MathPrim.getMathConfig().mathPenalty(left, right) : Num.NULL;
    }
    ...
    public MathWordBuilder getWordBuilder(byte fam, TreatNode proc) {
      FontMetric metric = fetchFontMetric(fam);
      return (metric.isNull()) ? MathWordBuilder.NULL : metric.getMathWordBuilder(proc);
    }

    public boolean forcedItalCorr(byte fam) {
      return fetchFontMetric(fam).getDimenParam(FontMetric.DIMEN_PARAM_SPACE).isZero();
    }
  }
}
