...
public abstract class Paragraph extends TypoCommand {
  ...
  public static void makeIndent(Builder bld) {
    bld.addBox(
        new HBoxNode(
            new BoxSizes(
                Dimen.ZERO, getConfig().getDimParam(DIMP_PAR_INDENT), Dimen.ZERO, Dimen.ZERO),
            GlueSetting.NATURAL,
            NodeList.EMPTY));
  }
  ...
  public static void start(boolean indent) {
    ...
    Builder par = new ParBuilder(currLineNumber(), getTypoConfig().getLanguage());
    Builder.push(par);
  ...
  public static void lineBreak(
    ...
    if (list.isEmpty()) Builder.pop();
      ...
      if (left.getShrOrder() != Glue.NORMAL && !left.getShrink().isZero()
          || right.getShrOrder() != Glue.NORMAL && !right.getShrink().isZero()) {
      ...
      Node leftSkip =
          (left.isZero()) ? Node.NULL : new NamedHSkipNode(left, cfg.getGlueName(GLUEP_LEFT_SKIP));
      Node rightSkip = new NamedHSkipNode(right, cfg.getGlueName(GLUEP_RIGHT_SKIP));
      ...
      if (breaker.hasMoreLines()) {
        ...
        for (; ; ) {
          ...
          NodeEnum mig =
              (bld.wantsMigrations()) ? line.extractedMigrations().nodes() : NodeList.EMPTY_ENUM;
        ...
        if (lastVisibleWidth != Dimen.NULL_PAR) {
          ...
          if (visible != Dimen.NULL) visible = visible.plus(lastIndent);
  ...
  private static class HyphNodeEnum extends HyphenNodeEnum {
    ...
    protected CharCode hyphenChar(FontMetric metric) {
      ...
      return (num != Num.NULL) ? Token.makeCharCode(num.intVal()) : CharCode.NULL;
    }
