...
public abstract class BaseNoad implements Noad {
  ...
  /* TeXtp[756] */
  protected static Egg makeScriptsTo(
      Egg body, boolean notJustChar, Converter conv, Field sup, Field sub) {
    ...
    if (!sup.isEmpty()) {
      ...
      if (notJustChar) {
        supShift =
            body.getHeight().max(Dimen.ZERO).minus(conv.getDimPar(DP_SUP_DROP, SUPER_SCRIPT));
      }
    }
    if (!sub.isEmpty()) {
      subNode = makeScriptBox(sub, conv, SUB_SCRIPT);
      if (notJustChar) {
        subShift = body.getDepth().max(Dimen.ZERO).plus(conv.getDimPar(DP_SUB_DROP, SUB_SCRIPT));
      }
      ...
      supShift =
          supShift.max(
              conv.getDimPar(
                  (conv.isCramped())
                      ? DP_SUP3
                      : (conv.getStyle() == Noad.DISPLAY_STYLE) ? DP_SUP1 : DP_SUP2));
      supShift = supShift.max(supNode.getDepth().plus(xHeight.over(4)));
      if (subNode == Node.NULL) scripts = HShiftNode.shiftingUp(supNode, supShift);
      else {
