...
public abstract class BaseFontMetric implements FontMetric {
  ...
  public Num getNumParam(int idx) {
    return (0 <= idx && idx < numPars.length) ? numPars[idx] : Num.NULL;
  }

  public Num setNumParam(int idx, Num val) {
    return (0 <= idx && idx < numPars.length) ? (numPars[idx] = val) : Num.NULL;
  }

  public boolean definesNumParams(int[] idxs) {
    for (int i = 0; i < idxs.length; i++) {
      int idx = idxs[i];
      if (idx < 0 || idx >= numPars.length || numPars[idx] == Num.NULL) return false;
    }
    return true;
  }
  ...
  public Dimen getDimenParam(int idx) {
    return (0 <= idx && idx < dimPars.length) ? dimPars[idx] : Dimen.NULL;
  }
  ...
  public boolean definesDimenParams(int[] idxs) {
    for (int i = 0; i < idxs.length; i++) {
      int idx = idxs[i];
      if (idx < 0 || idx >= dimPars.length || dimPars[idx] == Dimen.NULL) return false;
    }
    return true;
  }
