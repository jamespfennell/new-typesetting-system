...
public abstract class	PageSplit	extends VertSplit {
	...
	discarding = true; nullSpecs = true; last = Node.NULL;
	insMap.clear(); maybeOver.clear();
    }

    { restart(); }

    protected void	freezeSpecs()
	{ if (nullSpecs) { initSpecs(); nullSpecs = false; } }
    ...
    public boolean	canChangeDimens() { return (!nullSpecs); }
    protected boolean	waitingForMore() { return true; }

    protected void	passWhilePrunning(Node node) { pass(node); }
    protected void	passWhileBreaking(Node node) { pass(node); }

    public int			insertPenalties = 0;

    protected int	extraPenalty() { return insertPenalties; }
	...
	public void	markBest() { bestCount = data.size(); }

	/* TeXtp[1008] */
	public void	record(Insertion ins) {
	    Dimen	delta = goal.plus(soFar.getShrink())
				    .minus(soFar.getNatural()).minus(depth);
	...
	private void	split(Insertion ins) {
	    ...
	    InsertSplit	splitter
		= new InsertSplit(ins.list.nodes(), ins.topSkip);
	    ...
	    int		cost = (breakNode == Node.NULL) ? Node.EJECT_PENALTY
	    		     : (breakNode.isPenalty())
			     ? breakNode.getPenalty().intVal() : 0;
	    traceSplitCost(num, space, bestSize, cost);
	    full = true; size = size.plus(bestSize);
	    goal = goal.minus(insScale(bestSize, num));
	    data.add(head); insertPenalties += cost;
	    if (!splitter.isEmpty()) {
		splitter.pruneTop();
		NodeList	tail = new NodeList(splitter.nodes());
		VBoxNode	vbox = VBoxNode.packedOf(tail);
		maybeOver.append(new InsertNode(ins.makeCopy(tail,
			    vbox.getHeight().plus(vbox.getDepth()))));
	    }
	}
	...
	public void	finish() {
		...
		for (int i = 0; i < bestCount; i++)
		    list.append((NodeList) data.get(i));
	...
	public void	show(Log log) {
	    log.endLine().addEsc("insert").add(num).add(" adds ")
	...
	public InsertSplit(NodeEnum nodes, Glue topSkip)
	    { super(nodes); this.topSkip = topSkip; }

	private Dimen		bestSize;

	public Dimen	getBestSize() { return bestSize; }
    ...
    protected abstract void	traceSplitCost(int num, Dimen space,
					       Dimen best, int cost);
    protected abstract Node	splitTopAdjustment(Dimen height,
    						   Glue topSkip);
	...
	heldOver.append(maybeOver); maybeOver.clear();
	bestGoal = goal;
    }
	...
	if (discarding) {
	    ...
	    if (pruned) { freezeSpecs(); discarding = false; }
	    else return false;
	}
	return findBreak();
    }
    ...
    public void		build() {
	    ...
	    } else { insertPenalties = 0; heldOver.clear(); }
	    restart();
	...
	data = new Vector();
	append(heldOver); heldOver.clear();
	append(output); append(contrib);
    }

    public void		startNextPage() { startNextPage(EMPTY_ENUM); }
