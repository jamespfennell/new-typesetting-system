...
public abstract class AnyBoxNode extends AnyBoxedNode implements Box {
  ...
  private void typeSet(TypeSetter setter, boolean allowIO) {
    ...
    moveStart(setter);
    SettingContext sctx = new SettingContext(sizes, setting, setter.mark(), allowIO);
  ...
  public void addOn(Log log, CntxLog cntx, Dimen shift) {
    ...
    if (shift != Dimen.NULL && !shift.isZero()) log.add(", shifted ").add(shift.toString());
