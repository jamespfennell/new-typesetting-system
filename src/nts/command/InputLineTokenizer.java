...
/**
 * The |InputLineTokenizer| reads internal character codes from a |LineReader| and converts them
 * into |Token|s.
 */
public class InputLineTokenizer extends Tokenizer {
  ...
  /**
   * Creates an |InputLineTokenizer| with given input and parametrization object.
   *
  ...
  /**
   * A |TokenMaker| is used for parametrization of tokenization process. It provides parameters
  ...
  /**
   * Gives the |Token| constructed from the next internal character code read from input line or
   * from a sequence of such codes.
   *
   * @return the next token from the input line or |Token.NULL| if the input line is finished.
   */
  public Token nextToken() {
    for (; ; ) {
      CharCode code = line.getNext();
      if (code == InputLine.EOL) return Token.NULL;
      else
          ...
          case ESCAPE:
            ...
            if (code == InputLine.EOL) {
              ...
              if (cat != LETTER) {
                ...
                buf.append(code);
                while ((code = line.peekNext()) != InputLine.EOL && maker.scanCat(code) == LETTER) {
                  buf.append(code);
                  line.getNext();
                }
                state = SKIP_BLANKS;
