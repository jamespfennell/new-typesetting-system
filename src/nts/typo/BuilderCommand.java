...
public abstract class BuilderCommand extends TypoCommand {
  ...
  public final Glue getSkipForLeaders() {
    Builder bld = getBld();
    Action act = getAction(bld.getClass());
    return (act == Action.NULL) ? getSkipForLeaders(bld) : act.getSkipForLeaders(bld);
  }
  ...
  public Glue getSkipForLeaders(Builder bld) {
    return Glue.NULL;
  }
  ...
  public static void handleSpace(Token src) {
    Builder bld = getBld();
    CharHandler hnd = getCharHandler(bld.getClass());
    if (hnd == CharHandler.NULL) error("CantUseIn", str("space"), bld);
