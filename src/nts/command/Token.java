...
public abstract class Token implements Serializable, Loggable {
  ...
  /**
   * Tells whether this |Token| was not created from the input but was inserted during some error
   * recovery.
   *
  ...
  public void define(Command cmd, boolean glob) {
    throw new RuntimeException("Attempt to redefine meaning of a non-definable Token");
  }
  ...
  /**
   * Tests whether this |Token| matches normal (letter or other) char of particular char code.
   *
  ...
  /**
   * Gives the 7 bit ascii character code of letter character |Token|. The result is
   * |CharCode.NO_CHAR| if the |Token| is not a letter or if its character code is not a 7 bit
   * ascii.
   *
  ...
  /**
   * Gives the 7 bit ascii character code of other character |Token|. The result is
   * |CharCode.NO_CHAR| if the |Token| is not an other or if its character code is not a 7 bit
   * ascii.
   *
   * @return the 7 bit ascii code if the category is matched and the code is defined,
   *     |CharCode.NO_CHAR| otherwise.
  ...
  /**
   * Gives the 7 bit ascii character code of normal (letter or other) character |Token|. The result
   * is |CharCode.NO_CHAR| if the |Token| is not normal (letter or other) or if its character code
  ...
  /**
   * Gives the |CharCode| of non active character |Token|. The result is |CharCode.NULL| if the
   * |Token| is not a character or is an active character,
   *
   * @return the |CharCode| if the category is matched and the code is defined, |CharCode.NULL|
   *     otherwise.
   */
  ...
  /**
   * Gives the 7 bit ascii character code of non active character |Token|. The result is
   * |CharCode.NO_CHAR| if the |Token| is active character, is not a character or if its character
