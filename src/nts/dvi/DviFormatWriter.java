...
public class DviFormatWriter {
  ...
  public DviFormatWriter(OutputStream out, int num, int den, int mag, byte[] comm, int size) {
    if (out instanceof DviOutputStream) this.out = (DviOutputStream) out;
  ...
  private void checkPage() {
    if (!pageStarted) throw new RuntimeException("page not started in DVI");
  }
  ...
  public void startPage(int[] pars) {
    if (!pageStarted) {
      int loc = currLoc();
      outByte(BOP);
      for (int i = 0; i < 10; i++) outWord((i < pars.length) ? pars[i] : 0);
      outWord(lastPageLoc);
      numOfPages++;
      lastPageLoc = loc;
      pageStarted = true;
  ...
  public void pop() {
    checkPage();
    if (stack != Level.NULL) {
      xStack.top = stack.xTop;
      yStack.top = stack.yTop;
  ...
  protected void defFont(int font, FontDef def) {
    outPack(FNT_DEF1, font);
    outWord(def.chksum);
    outWord(def.size);
    outWord(def.dsize);
    outByte(def.dir.length);
    outByte(def.name.length);
    outBytes(def.dir);
    outBytes(def.name);
  }
  ...
  public void close(int maxH, int maxW) {
    ...
    outWord(maxH);
    outWord(maxW);
    outHalf(maxPushLevel);
    outHalf(numOfPages);
    Object[] keys = fonts.keySet().toArray();
    Arrays.sort(keys);
    int i = keys.length;
    while (i-- > 0) defFont(((Integer) keys[i]).intValue(), (FontDef) fonts.get(keys[i]));
    outByte(POST_POST);
    outWord(loc);
    outByte(ID_BYTE);
    i = 4 + (5 - (currLoc() + 1 & 3) & 3);
