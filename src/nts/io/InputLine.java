...
public class InputLine implements Loggable {
  ...
  /**
   * Creates |InputLine| for given |String| using an |Mapper|. It ignores trailing characters which
   * should be ignored (usually spaces) and appends end line character if defined.
   *
  ...
  public InputLine(String str, Mapper mapper) {
    ...
    this.mapper = mapper;
    pos = 0;
    for (int i = 0; i < end; i++) codes[i] = mapper.map(str.charAt(i));
  }
  ...
  public boolean wasEmpty(boolean addEolc) {
    if (addEolc) {
      CharCode elc = mapper.endLine();
      if (elc != CharCode.NULL) return (codes.length == 1 && codes[0].match(elc));
    }
  ...
  /**
   * Gives the next internal character code in the line. Interprets the expanded chracter
   * constructions ('^^').
   *
   * @return the next internal character code or |EOL| if the line is finished.
   */
  /* TeXtp[352,355] */
  ...
  /**
   * Tests whether given character is a lowercase hexadecimal digit which can be part of hexadecimal
   * expanded character construction.
   *
  ...
  /**
   * Gives the number corresponding to lowercase hexadecimal digit. The parameter must be tested by
   * method |isHexDigit|, this method does not check the validity.
   *
  ...
  /**
   * Gives the internal character code on the current position. It does not advance the current
   * position.
   *
   * @return the next internal character code or |EOL| if the line is finished.
   */
  ...
  public synchronized CharCode peekNext() {
    ...
    if (pos > oldPos + 1) {
      CharCode[] newCodes = new CharCode[oldPos + 1 + codes.length - pos];
      System.arraycopy(codes, 0, newCodes, 0, oldPos);
      newCodes[oldPos] = code;
      System.arraycopy(codes, pos, newCodes, oldPos + 1, codes.length - pos);
      codes = newCodes;
    }
    pos = oldPos;
    return code;
