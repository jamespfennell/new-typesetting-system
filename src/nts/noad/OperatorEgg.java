...
public class OperatorEgg extends Egg {
  ...
  public Dimen getHeight() {
    return (node == Node.NULL) ? Dimen.ZERO : node.getHeight().max(Dimen.ZERO).minus(shift);
  }

  public Dimen getDepth() {
    return (node == Node.NULL) ? Dimen.ZERO : node.getDepth().max(Dimen.ZERO).plus(shift);
  }

  public void chipShell(Nodery nodery) {
    Box box = (node == Node.NULL) ? HBoxNode.EMPTY : HBoxNode.packedOf(node);
    Dimen ital = getItalCorr();
    if (ital != Dimen.NULL) box = box.pretendingWidth(box.getWidth().plus(ital));
