...
public class LastSkipPrim extends BuilderPrim implements Glue.Provider {

  public LastSkipPrim(String name) {
    super(name);
  }
  ...
  public Glue getGlueValue() {
    Node node = getBld().lastSpecialNode();
    return (node != Node.NULL && node.isSkip()) ? node.getSkip() : Glue.ZERO;
  }

  /* TeXtp[424] */
  public Glue getMuGlueValue() {
    Node node = getBld().lastSpecialNode();
    return (node != Node.NULL && node.isMuSkip()) ? node.getMuSkip() : Glue.ZERO;
  }
}
