...
/**
 * Prim command of the macro language. It can be invoked by its name using control sequence Token.
 */
public abstract class Prim extends Command implements Primitive {
  ...
  protected static Dimen performFor(Dimen val, int operation) {
    ...
    try {
      switch (operation) {
        case ADVANCE:
          val = val.plus(scanDimen());
          break;
        case MULTIPLY:
          val = val.times(scanInt());
          break;
        case DIVIDE:
          val = val.over(scanInt());
          break;
      }
    } catch (ArithmeticException e) {
      val = Dimen.ZERO;
  ...
  public static TokenList scanTokenList(Loggable src, boolean xpand) {
    ...
    InpTokChecker savedChk =
        setTokenChecker(new ScanToksChecker("OuterInToks", "EOFinToks", "text", buf, src));
    scanLeftBrace();
    for (int balance = 1; ; ) {
