...
public class Preamble {
  ...
  public Preamble(
      Glue firstSkip, String skipName, Token endTemplate, Entry[] records, int loopIndex) {
    this.firstSkip = firstSkip;
    this.skipName = skipName;
    this.endTemplate = endTemplate;
    this.records = records;
    this.loopIndex = loopIndex;
    if (loopIndex >= records.length) throw new RuntimeException("invalid Preamble loopIndex");
  }
  ...
  private Entry getEntry(int i) {
    if (i >= 0) {
      if (i < records.length) return records[i];
        ...
        return records[loopIndex + (i - loopIndex) % (records.length - loopIndex)];
    }
    return NULL_ENTRY;
  }
