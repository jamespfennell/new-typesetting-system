...
public class OpNoad extends WordPartNoad {
  ...
  private Egg makeEgg(Operator oper, Converter conv, boolean display) {
    return (usingLimits(conv, display))
        ? new StSimpleNodeEgg(VBoxNode.packedOf(oper.getNodeToBeLimited()), spacingType())
  ...
  private Egg makeEggWithScripts(Operator oper, Converter conv, Field sup, Field sub) {
    ...
    if (supNode != Node.NULL) {
      ...
      Dimen clr = conv.getDimPar(DP_BIG_OP_SPACING3).minus(supNode.getDepth());
      ...
      list.append((delta == Dimen.NULL) ? supNode : VShiftNode.shiftingRight(supNode, delta));
    ...
    if (subNode != Node.NULL) {
      ...
      Dimen clr = conv.getDimPar(DP_BIG_OP_SPACING4).minus(subNode.getHeight());
      ...
      list.append(new IntVKernNode(clr));
      list.append((delta == Dimen.NULL) ? subNode : VShiftNode.shiftingLeft(subNode, delta));
      list.append(new IntVKernNode(extra));
    }
    return new StSimpleNodeEgg(
        new VBoxNode(new BoxSizes(height, width, depth, leftX), GlueSetting.NATURAL, list),
        spacingType());
  }
}
