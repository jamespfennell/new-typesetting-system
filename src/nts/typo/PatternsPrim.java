...
public class	PatternsPrim	extends TypoAssignPrim {
    ...
    public PatternsPrim(String name) { super(name); }
	...
	for (;;) {
		...
		if (toAdd != CharCode.NULL && code.match(toAdd)) {
			...
			if (code.match('.')) letter = WORD_BOUNDARY;
				...
				{ error("NonLetter"); letter = WORD_BOUNDARY; }
			}
			buf.append(letter); digitExpected = true;
		    }
		    continue;
		}
	    }
		...
		if (size > 0) {
			...
			if (  buf.charAt(size - 1) == WORD_BOUNDARY
			   && values.length > size  ) values[size] = 0;
		    }
		    if (!cfg.getLanguage().setHyphPattern(
						    buf.toString(), values))
			error("DupPattern");
		    buf.setLength(0); valBuf.clear();
		}
