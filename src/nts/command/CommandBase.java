...
public abstract class CommandBase {
  ...
  private static void setupLogs(LineOutput term, LineOutput file, boolean ten) {
    ...
    if (term != terminal || file != logFile)
      both =
          (term != LineOutput.NULL && file != LineOutput.NULL)
              ? new DoubleLineOutput(term, file)
              : (term != LineOutput.NULL) ? term : file;
    if (ten != termEnable || term != terminal)
      termLog = (ten && term != LineOutput.NULL) ? makeLog(term) : NullLog.LOG;
    if (file != logFile) fileLog = (file != LineOutput.NULL) ? makeLog(file) : NullLog.LOG;
    if (ten != termEnable || both != termAndLog)
      normLog = (!ten) ? fileLog : (both != LineOutput.NULL) ? makeLog(both) : NullLog.LOG;
  ...
  public abstract static class NumKind extends EqTable.NumKind implements Serializable {
  ...
  public abstract static class TokKind extends EqTable.ObjKind implements Serializable {
  ...
  public abstract static class ExtEquiv implements EqTable.ExtEquiv, EqTraceable {
  ...
  public static int scanInt() {
    return scanInt(IntPar.NULL);
  }

  public static Num scanNum() {
    return Num.valueOf(scanInt(IntPar.NULL));
  }

  public static Dimen scanDimen() {
    return scanDimen(false, BytePar.NULL);
  }

  public static Dimen scanMuDimen() {
    return scanDimen(true, BytePar.NULL);
  }
    ...
    if (tok.matchOther('`')) {
      ...
      for (boolean ok = true; ; tok = nextExpToken()) {
        ...
        if (dig != CharCode.NO_CHAR) {
          ...
          dig = tok.letterChar();
          if (dig != CharCode.NO_CHAR && dig >= 'A') digit = (dig - 'A') + 10;
        }
      ...
      if (cmd.hasNumValue()) {
        ...
        if (point && radix.get() == 10) {
          ...
          for (; ; ) {
            tok = nextExpToken();
            char dig = tok.otherChar();
            if (dig != CharCode.NO_CHAR && dig >= '0' && dig <= '9') buf.append(dig);
            else break;
          }
          try {
  ...
  private static Dimen convToDimenUnit(int whole, Dimen fract, boolean mu, BytePar glueOrder) {
    if (glueOrder != BytePar.NULL) {
      ...
      if (glueOrder.get() != Glue.NORMAL) return fract.plus(whole);
    }
  ...
  private static Dimen convToDimenUnit(int whole, Dimen fract, boolean mu) {
    ...
    if (mu) {
      if (cmd.hasMuDimenValue()) val = cmd.getMuDimenValue();
        ...
        val = Dimen.valueOf(cmd.getNumValue().intVal(), DIMEN_DENOMINATOR);
        muError();
      }
    } else {
      if (cmd.hasDimenValue()) val = cmd.getDimenValue();
    ...
    for (int i = 0; i < dimUnits.length; i++)
      if (scanKeyword(dimUnits[i].id)) {
        ...
        return (den != 0)
            ...
            : Dimen.shiftedValueOf(whole, num).plus(fract.shifted(num));
      }
    error("IllegalUnit");
    return fract.plus(whole);
  }
  ...
  protected static Glue scanGlue(boolean mu) {
    ...
    if (val != Glue.NULL) return (negative.get()) ? val.negative() : val;
    if (width == Dimen.NULL) {
      if (cmd.hasNumValue()) {
        int n = cmd.getNumValue().intVal();
        if (n < 0) {
  ...
  protected static FileName scanFileName() {
    ...
    for (; ; ) {
      ...
      i = (code != CharCode.NULL) ? name.accept(code) : -1;
      if (i <= 0) break;
      tok = nextExpToken();
      cmd = meaningOf(tok);
    }
