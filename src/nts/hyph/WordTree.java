...
public class WordTree implements WordMap {
  ...
  private class Enum implements Enumeration {
    ...
    public Object nextElement() {
      if (curr == NULL_ENTRY) throw new NoSuchElementException("WordTree is finished");
      Object mapEnt =
          new Map.Entry() {
            ...
            private String makeKey() {
              ...
              int i;
              for (i = 0; i < stack.size(); i++) codes[i] = ((Entry) stack.get(i)).code;
              codes[i] = curr.code;
  ...
  */
  ...
  private class Packer {
    ...
    private void distribute(Pack pack, int start) {
      do {
        table[start + pack.code] = new WordTrie.Entry(pack.code, pack.start, pack.value);
        if (pack.start != 0) distribute(pack.below, pack.start);
        pack = pack.next;
    ...
    private int allocate(Pack pack) {
      ...
      do {
        if (pack.below != NULL_PACK && pack.start == 0) pack.start = allocate(pack.below);
        pack = pack.next;
      } while (pack != NULL_PACK);
      return start;
    }
