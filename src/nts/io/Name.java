...
/**
 * This class represents a name consisting of internal characer codes. It is used for control
 * sequence names, names of primitive commands, etc.
 */
public class Name implements Serializable, Loggable {
  ...
  public static class Buffer implements Serializable {
    ...
    public int hashCode() {
      int h = 0;
      for (int i = 0; i < length(); i++) h = h * 39 + codeAt(i).hashCode();
      return h;
    }

    /**
     * Compares this internal character string to the specified object. The result is |true| if and
     * only if the the argument is not |null| and is the |Name| object representing the same
    ...
    public final boolean equals(Object o) {
      if (o != null && o instanceof Name.Buffer) {
        ...
        if (length() == other.length()) {
          for (int i = 0; i < length(); i++) if (!codeAt(i).equals(other.codeAt(i))) return false;
          return true;
        }
      }
      return false;
  ...
  /**
   * Gives an internal character code at the given index. An index ranges from |0| to |length() -
   * 1|.
   *
  ...
  /**
   * Compares this internal character string to the specified object. The result is |true| if and
   * only if the the argument is not |null| and is the |Name| object representing the same internal
   * character string.
   *
   * @param o the object to compare this internal character string against.
   * @return |true| if the argument is equal, |false| otherwise.
  ...
  public boolean match(Name x) {
    if (length() == x.length()) {
      for (int i = 0; i < length(); i++) if (!codeAt(i).match(x.codeAt(i))) return false;
      return true;
    }
    return false;
  }
