...
public abstract class AnyLeadersPrim extends BuilderPrim {
  ...
  public AnyLeadersPrim(String name) {
    super(name);
  }
  ...
  public void exec(final Builder bld, Token src) {
    ...
    if (cmd.hasBoxValue()) addBoxLeaders(bld, cmd.getBoxValue());
      ...
      Glue skip = getSkip();
      if (skip != Glue.NULL) bld.addLeadRule(skip, sizes, getDesc());
    } else {
      backToken(tok);
      error("BoxExpected");
    }
  ...
  protected void addBoxLeaders(Builder bld, Box box) {
    if (!box.isVoid()) {
      ...
      if (skip != Glue.NULL) {
        ...
        if (mover != BoxLeaders.NULL_MOVER) bld.addLeaders(skip, makeLeaders(box, mover));
      }
    }
  }
  ...
  protected abstract Leaders makeLeaders(Node node, BoxLeaders.Mover mover);
}
